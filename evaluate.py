...
#就是计算test和val的得分，只通过真实值和预测值，逐特征做loss
def get_full_err_scores(test_result, val_result):
    ...
    for i in range(feature_num):
        ...
        scores = get_err_scores(test_re_list, val_re_list)#计算test的分数
        normal_dist = get_err_scores(val_re_list, val_re_list)#计算val的分数
...
#只计算了test的，没有对val进行处理，通过真实值和预测值做loss
...
def evaluate_mse(predicted_values, true_values):
    mse = mean_squared_error(true_values, predicted_values)
    return mse


def evaluate_mae(predicted_values, true_values):
    mae = mean_absolute_error(true_values, predicted_values)
    return mae


def evaluate_r2(predicted_values, true_values):
...
def calculate_mape(predicted_values, true_values):
    absolute_percentage_errors = np.abs((true_values - predicted_values) / true_values)
    # Masking values greater than 10000
    absolute_percentage_errors = np.where(absolute_percentage_errors > 10000, np.nan, absolute_percentage_errors)
    # Removing NaN values
