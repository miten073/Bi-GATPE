...
class GraphLayer(MessagePassing):
    ...
    def reset_parameters(self):
        glorot(self.lin.weight) # 使用Glorot方法初始化线性层的权重
    ...
    def forward(self, x, edge_index, embedding, return_attention_weights=False):
        ...
        # # 获取元组的长度（元素数量）
        # num_elements = len(x)
        # print("Number of elements in the tuple:", num_elements)
    ...
    def message(self, x_i, x_j, edge_index_i, size_i, embedding, edges, return_attention_weights):
        ...
        # 先将 key_i 和 key_j 相加，然后应用 LeakyReLU
        alpha = F.leaky_relu((key_i + key_j), self.negative_slope)


        # 与 cat_att_i 和 cat_att_j 相乘，然后按最后一个维度求和
        ...
        alpha = softmax(alpha, edge_index_i, size_i) # 对alpha进行softmax归一化
